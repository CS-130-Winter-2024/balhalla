...
function AvatarSelector ({initImageName = "Gus"}) {
  ...
  const [selectedAvatar, setSelectedAvatar] = useState(descriptionMap.indexOf(initImageName) !== -1 ? imageOptions[descriptionMap.indexOf(initImageName)] : initImageName);
    ...
    <Box>
      <Tooltip title="Change Avatar">
        <IconButton onClick={handleOpen} sx={styles.avatarButton}>
          <Avatar src={selectedAvatar} alt="Profile Avatar" sx={styles.avatarImage} />
        </IconButton>
      </Tooltip>
          ...
          <Grid container spacing={2}>
            {imageOptions.map((imageUrl) => (
              <Grid item key={imageUrl}>
                ...
                >
                  <Avatar src={imageUrl} alt="Profile Option" sx={{ width: 120, height: 120, marginBottom: 1 }} />
                  <Typography variant="caption">{descriptionMap[imageOptions.indexOf(imageUrl)]}</Typography>
                </Box>
              </Grid>
            ))}
          </Grid>
