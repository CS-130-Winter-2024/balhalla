...
import { add_listener, get_global, set_global, remove_listener } from "../../constants";
...
export default function Clock({}) {
    ...
    useEffect(()=>{
        ...
        return ()=>{remove_listener("CURRENT_TIMER",listener);remove_listener("TIMER_LABEL",textListener);}
    },[])
    ...
    useEffect(()=>{
        ...
        set_global("TIMEOUT_ID",setTimeout(()=>{
            let tDiff = time - Date.now();
            setDisplay(Math.ceil(tDiff/1000));
            set_global("CLOCK_ID",setInterval(
                ()=>{
                    let tDiff = time - Date.now();
                    setDisplay(Math.ceil(tDiff/1000));
                },
